...
void SingleInferenceData::ProcessResult() {
  ...
  sem_t *infer_semaphore = sem_open("/CoreDLA_infer",0);
  sem_t *post_semaphore = sem_open("/CoreDLA_post",0);
  sem_post(infer_semaphore);
  _numResults++;
  if (_numResults == 1) {
  ...
  float* shared = (float*) mmap(NULL, outputSize*4, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

  for (size_t i = 0; i < outputSize; i++) {
     shared[i] = pOutputData[i];
  }
  sem_post(post_semaphore);


  std::stringstream outString;
  outString <<"RESULT!  size = " << outputSize << '\n';


  std::cout << outString.str();
  sem_close(post_semaphore);
  sem_close(infer_semaphore);
